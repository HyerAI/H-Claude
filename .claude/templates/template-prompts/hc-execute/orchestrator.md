...
AGENT_OUTPUT=$(timeout --foreground --signal=TERM --kill-after=30 1200 \
  bash -c 'ANTHROPIC_API_BASE_URL=http://localhost:2412 claude --dangerously-skip-permissions -p "PROMPT"' 2>&1)
AGENT_EXIT=$?
...
agent: EXECUTE_ORCHESTRATOR
session: ${SESSION_SLUG}
started: ${SESSION_START}
last_beat: $(date -Iseconds)
status: WORKING
current_phase: ${CURRENT_PHASE}
phases_complete: ${PHASES_COMPLETE}/${TOTAL_PHASES}
EOF
}
...
Include in Oraca/Worker prompts:
