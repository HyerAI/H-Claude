...
tags: [command, execution, orchestration, plan, oraca]
...
PLAN_PATH: [path to approved plan, e.g., .claude/PM/plans/{session-slug}/APPROVED_PLAN.md]
...
This command is designed to follow `/hc-plan`:

```
/hc-plan → APPROVED_PLAN.md
                ↓
/hc-plan-execute PLAN_PATH=[...]/APPROVED_PLAN.md
                ↓
        COMPLETION_REPORT.md
```

Session folders pair naturally:
- `.claude/PM/plans/{session-slug}/` (plan outputs)
- `.claude/PM/hc-plan-execute/{session-slug}/` (execution outputs)
...
|---------|---------------------|
| `/hc-plan` | Planning phase (produces the plan to execute) |
